...
function GIFEncoder(width, height) {
  ...
  this.actorImage = null; // current frame
...
GIFEncoder.prototype.addFrame = function(imageData) {
  ...
  if (imageData && imageData.getImageData) {
    this.actorImage = imageData.getImageData(0, 0, this.width, this.height).data;
  } else {
    this.actorImage = imageData;
  }
...
GIFEncoder.prototype.analyzePixels = function() {
  ...
  this.colorTab = [0,0,0];
  var palette = {}, palIndex = 1;

  var k = 0;
  for (var j = 0; j < nPix; j++) {
    var r = this.pixels[k++], g = this.pixels[k++], b = this.pixels[k++];
    var colorValue = (r << 16) + (g << 8) + b;
    var hex = '#'+colorValue.toString(16);
    var thisColor = 0;
    if(!(palette[hex] >= 0)) {
      if(colorValue != this.transparent) {
        palette[hex] = palIndex++;
        this.colorTab.push(r);
        this.colorTab.push(g);
        this.colorTab.push(b);
        thisColor = palette[hex];
      }
    } else {
      thisColor = palette[hex];
    }
    this.indexedPixels[j] = thisColor;
  }
